...
import ba.unsa.etf.rma.adnan_alibegovic.rma15_16593.Fragments.MainScreenFragment;
import ba.unsa.etf.rma.adnan_alibegovic.rma15_16593.Fragments.MusicianDetailsFragment;
import ba.unsa.etf.rma.adnan_alibegovic.rma15_16593.Fragments.MusiciansListFragment;
...
public class MainActivity extends AppCompatActivity implements MusiciansListFragment.OnMusicianItemClick {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // getting musicians fromdatabase if the list is empty
        if(musicians.size() == 0){
            MusicianDBOpenHelper dbHelper = MusicianDBOpenHelper.getInstance(getApplicationContext());
            musicians = dbHelper.getAllMusicians(5);
            if(musicians == null) {
        ...
        if(fragmentManager.findFragmentById(R.id.musicians_list_frame) == null) {
            MainScreenFragment musiciansFragment = new MainScreenFragment();
            Bundle args = new Bundle();
...
//        }
    ...
    public void onMusicianItemClicked(int pos) {
        ...
        //inserting musician in database
        MusicianDBOpenHelper helper = MusicianDBOpenHelper.getInstance(getApplicationContext());
        helper.addOrUpdateMusician(musicians.get(pos));
